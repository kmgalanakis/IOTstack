...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Containers/Glances'
  ...
  def onResize(sig, action):
    global glancesBuildOptions
    global currentMenuItemIndex
    mainRender(1, glancesBuildOptions, currentMenuItemIndex)
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack Glances Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global glancesBuildOptions
    if len(glancesBuildOptions[selection]) > 1 and isinstance(glancesBuildOptions[selection][1], types.FunctionType):
      glancesBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, glancesBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(glancesBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(glancesBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(glancesBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'glances':
  main()
else:
  print("Error. '{}' Tried to run 'glances' config".format(currentServiceName))