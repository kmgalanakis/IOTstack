...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Containers/ResilioSync'
  ...
  def onResize(sig, action):
    global resiliosyncBuildOptions
    global currentMenuItemIndex
    mainRender(1, resiliosyncBuildOptions, currentMenuItemIndex)

  resiliosyncBuildOptions = []
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack ResilioSync Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global resiliosyncBuildOptions
    if len(resiliosyncBuildOptions[selection]) > 1 and isinstance(resiliosyncBuildOptions[selection][1], types.FunctionType):
      resiliosyncBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, resiliosyncBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(resiliosyncBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(resiliosyncBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(resiliosyncBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'resiliosync':
  main()
else:
  print("Error. '{}' Tried to run 'resiliosync' config".format(currentServiceName))