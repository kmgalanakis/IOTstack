...
def main():
  ...
  documentationHint = 'https://sensorsiot.github.io/IOTstack/Containers/MiniDLNA'
  ...
  def onResize(sig, action):
    global minidlnaBuildOptions
    global currentMenuItemIndex
    mainRender(1, minidlnaBuildOptions, currentMenuItemIndex)

  minidlnaBuildOptions = []
  ...
  def mainRender(needsRender, menu, selection):
    ...
    if needsRender == 1:
      ...
      print(term.black_on_cornsilk4(term.center('IOTstack MiniDLNA Options')))
      print("")
  ...
  def runSelection(selection):
    import types
    global minidlnaBuildOptions
    if len(minidlnaBuildOptions[selection]) > 1 and isinstance(minidlnaBuildOptions[selection][1], types.FunctionType):
      minidlnaBuildOptions[selection][1]()
    else:
  ...
  def menuEntryPoint():
    ...
    with term.fullscreen():
      menuNavigateDirection = 0
      mainRender(needsRender, minidlnaBuildOptions, currentMenuItemIndex)
      selectionInProgress = True
      with term.cbreak():
        while selectionInProgress:
          ...
          if menuNavigateDirection != 0: # If a direction was pressed, find next selectable item
            ...
            currentMenuItemIndex = currentMenuItemIndex % len(minidlnaBuildOptions)
            needsRender = 2

            while not isMenuItemSelectable(minidlnaBuildOptions, currentMenuItemIndex):
              currentMenuItemIndex += menuNavigateDirection
              currentMenuItemIndex = currentMenuItemIndex % len(minidlnaBuildOptions)
    return True
...
global currentServiceName # Name of the current service
if currentServiceName == 'minidlna':
  main()
else:
  print("Error. '{}' Tried to run 'minidlna' config".format(currentServiceName))